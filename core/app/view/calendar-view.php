...
foreach($events as $event){
	if($event->type=="1"){
		...
		$thejson[] = array("title"=>$event->title,"color"=>"red","url"=>"#","start"=>$event->date_at."T".$event->time_at);
	}else{
		$thejson[] = array("title"=>$event->title,"color"=>"blue","url"=>"./?view=editfastreservation&id=".$event->id,"start"=>$event->date_at."T".$event->time_at);
	}
}
?>
<script>
